...
- Dreamhack 강의·Lab·문제의 원문이나 해설을 이 사이트에 복제하지 않는다.
- 사이트에는 일정, 과제 제출, 자체 피드백만 관리한다.
- 실습은 Dreamhack이 제공하는 환경이나 동아리가 명시적으로 허가한 격리 환경에서만 진행한다.
- 매주 수업 종료 후 현재 주차를 `done`, 다음 주차를 `active`로 변경하고 해당 과제를 공개한다.
- 일부 Unit과 Lab은 Dreamhack 유료 플랜 또는 Coin이 필요할 수 있으므로 개강 전에 전원의 접근 권한을 확인한다.

## 선수 지식 확인

이 과정은 웹 해킹 기초, HTML·JavaScript 기본 문법, SQL 및 DBMS 동작, HTTP와 웹 보안 메커니즘의 기초 이해를 전제로 한다. 선수 지식이 부족한 학생에게는 별도 보충 자료를 제공한다.

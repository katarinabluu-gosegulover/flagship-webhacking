(async function initAdmin() {
  ...
  function submissions() {
    return `${head('REVIEW QUEUE', '제출 검사', 'private 저장소의 제출 파일을 확인하고 점수와 피드백을 기록하세요.')}
      ...
      <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>STUDENT</th><th>ASSIGNMENT</th><th>FILE</th><th>SUBMITTED</th><th>STATUS</th><th>ACTION</th></tr></thead><tbody>${data.submissions.map((item) => `<tr data-search="${esc(`${item.profiles?.display_name} ${item.assignments?.title} ${item.original_file_name}`.toLowerCase())}" data-status="${item.status}"><td><div class="student"><span class="avatar">${esc(item.profiles?.display_name?.[0] || '?')}</span><strong>${esc(item.profiles?.display_name || '알 수 없음')}</strong></div></td><td>${esc(item.assignments?.title || '-')}</td><td>${esc(item.original_file_name)}</td><td>${date(item.submitted_at)}</td><td><span class="status ${item.status === 'graded' ? 'done' : 'wait'}">${item.status === 'graded' ? `${item.reviews?.[0]?.score ?? '-'}점` : '검사 대기'}</span></td><td class="actions"><button class="small-btn" data-view-file="${item.id}">파일</button><button class="small-btn" data-review-id="${item.id}">${item.status === 'graded' ? '수정' : '채점'}</button></td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function showDayActivity(dateKey) {
    ...
    openModal(`<span class="eyebrow">DAILY SUBMISSIONS / ${esc(selectedName)}</span><h2 id="modalTitle">${displayDate}</h2><p class="modal-desc">이날 제출된 과제 ${matches.length}건입니다.</p><div class="day-activity-list">${matches.map((item) => `<div class="day-activity-row"><span class="activity-icon">${esc(item.profiles?.display_name?.[0] || '?')}</span><div><strong>${esc(item.profiles?.display_name || '알 수 없음')} · ${esc(item.assignments?.title || '삭제된 과제')}</strong><small>${esc(item.original_file_name)} · ${date(item.submitted_at)}</small></div><span class="status ${item.status === 'graded' ? 'done' : 'wait'}">${item.status === 'graded' ? `${item.reviews?.[0]?.score ?? '-'}점` : '검사 대기'}</span></div>`).join('')}</div><div class="modal-actions"><button class="secondary-btn" data-close-day>닫기</button><button class="primary-btn" data-open-review-queue>제출 검사로 이동 →</button></div>`);
  ...
  function reviewModal(id) {
    const item = data.submissions.find((entry) => entry.id === id); const review = item.reviews?.[0];

(async function initStudentApp() {
  ...
  function dashboard() {
    ...
    const average = graded.length ? Math.round(graded.reduce((sum, item) => sum + (item.reviews?.[0]?.score || 0), 0) / graded.length) : '-';
  ...
  function assignmentRow(item) { const submission = submissionFor(item.id); const status = submission?.status || 'open'; const review = submission?.reviews?.[0]; return `<article class="assignment-row" data-status="${status}" data-search="${esc(`${item.code} ${item.title}`.toLowerCase())}"><span class="assignment-id">${esc(item.code)}</span><div class="assignment-info"><strong>${esc(item.title)}</strong><small>${esc(item.description)}</small></div><div class="assignment-cell"><span>DEADLINE</span><strong>${date(item.due_at)}</strong></div><div class="assignment-cell"><span>FILE</span><strong>${esc(submission?.original_file_name || '-')}</strong></div><div class="assignment-cell"><span>STATUS</span>${status === 'graded' ? `<strong class="score">${review?.score ?? '-'}점</strong>` : `<em class="status ${status === 'submitted' ? 'wait' : 'now'}">${status === 'submitted' ? '검사 중' : '제출 가능'}</em>`}</div><div class="assignment-actions">${status === 'open' ? `<button class="small-btn" data-submit-id="${item.id}">제출하기</button>` : status === 'graded' ? `<button class="small-btn" data-feedback-id="${submission.id}">피드백</button>` : '<button class="small-btn" disabled>피드백 대기</button>'}</div></article>`; }
